...
func StartTokenHealthCheck(database *sql.DB, intervalMinutes int) {
	go func() {
		...
		safe.Run("StartTokenHealthCheck.checkAllTokens", func() { checkAllTokens(database) })
...
func checkAllTokens(database *sql.DB) {
	...
	for _, acc := range accounts {
		account := acc
		func() {
			defer safe.Recover("checkAllTokens." + account.DisplayName)
			checkOneToken(database, account)
		}()
	}
}

func checkOneToken(database *sql.DB, account types.GameAccount) {
	...
	req, err := http.NewRequest("GET",
		...
		nil)
	if err != nil {
		slog.Error("HealthCheck: error construyendo request", "bot", account.DisplayName, "error", err)
		return
	}
